...
后续 AI 执行时，默认应直接读取本文件和 `CLAUDE.md` 作为主要规范来源；除非版式细节拿不准，否则不要依赖历史对话。最多只把温州后京最终成品作为版式参照。

`AGENTS.md` 与 `CLAUDE.md` 当前应保持内容基本一致。保留两份文件的目的主要是兼容不同 AI/代理入口：有的默认优先读 `AGENTS.md`，有的优先读 `CLAUDE.md`。后续更新规范时，默认两份同步修改，不要只改其中一份。
...
当 raw/ 内容与其他来源冲突时，优先以 raw/ 和用户最新口头说明为准。
